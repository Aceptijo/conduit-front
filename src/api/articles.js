...
export const getGlobalArticles = async (type, limit, offset) => {
  const endpoint = type === 'global' ? '/articles' : '/articles/feed';
  const response = await axiosInstance.get(endpoint, {
    params: {
      offset: offset,
      limit: limit,
...
export const getArticlesByTag = async (selectedTag, limit, offset) => {
